...
- `v7`：激进实验，尝试将 `topk_weights` 降为 bf16 以减少带宽。

## v6 错误结论（简要）

- 错误不是 crash，而是 correctness 大规模不通过。
- 直接原因：`doweight_stage1=True` 路径下结果与参考不一致（所有 benchmark case 均失败）。
- 处理：新增 `v6_fix`，不改历史 v6。

## 推荐提交顺序（时间紧）

1. 先提 `v5_fix` 或 `v6_fix`（确保稳定通过）
2. 再提 `v7`（冲击性能）
3. 若 `v7` 精度失败，立即回退 `v6_fix`

## 提交命令（moe）

```bash
popcorn-cli submit --mode benchmark --gpu MI355X --leaderboard amd-moe-mxfp4 moe-mxfp4/versions/submission_v6_fix.py --no-tui
popcorn-cli submit --mode benchmark --gpu MI355X --leaderboard amd-moe-mxfp4 moe-mxfp4/versions/submission_v7.py --no-tui
```
