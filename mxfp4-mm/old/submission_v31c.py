...
# ==========================================
# 1. Source Inject: fused quant+shuffle kernel
# ==========================================
...
try:
    with open(KERNEL_FILE, 'w') as f: f.write(KERNEL_CODE)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # ASM 路径：
    #   1) M>=128 且 K>=1536 — preshuffle 实测更慢 (20.7μs vs 18.3μs)
    #   2) M>=64  且 K<=512  — 大 M 小 K preshuffle 无合适 config，避免 KeyError
    use_asm = (M >= 128 and K >= 1536) or (M >= 64 and K <= 512)
    if use_asm:
    ...
    # preshuffle 路径：小 M（<64）所有 K
    # 直接传 config dict，完全绕过 JSON 文件 lookup
    ...
    if K <= 512:
        ...
        # K=7168，NUM_KSPLIT=14 来自 aiter 已有 tuned config
        cfg = {
            "BLOCK_SIZE_M": 16,
            "BLOCK_SIZE_N": 128,
            "BLOCK_SIZE_K": 512,
