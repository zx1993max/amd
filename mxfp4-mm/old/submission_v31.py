...
def custom_kernel(data: input_t) -> output_t:
    ...
    # ---- Convert B_shuffle to preshuffle kernel format ----
    w = B_shuffle.view(torch.uint8).reshape(N // 16, -1)

    # ---- Convert B_scale_sh to shuffle_scales format (N//32, K) ----
    bs = B_scale_sh.view(torch.uint8)
    N_pad, scaleN_pad = bs.shape
    w_scales = bs.reshape(N_pad // 32, 32 * scaleN_pad)[:N // 32, :K]

    # ---- Call preshuffle fused quant+GEMM with auto config lookup ----
    out = gemm_a16wfp4_preshuffle(A.contiguous(), w, w_scales, dtype=torch.bfloat16)
    return out
