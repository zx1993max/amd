...
# ==========================================
# 3. Entry point — hybrid: Triton fused for small K, ASM for large K
# ==========================================
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if K <= 512:
        # ---- Small K: Triton fused quant+GEMM (single kernel, no separate quant) ----
        ...
        return gemm_a16wfp4(A, B_q.view(torch.uint8), B_scale, config=cfg, dtype=torch.bfloat16)
    else:
        # ---- Large K: fused quant+shuffle + ASM GEMM (best GEMM perf) ----
