...
# ==========================================
# 2. Source inject: preshuffle config files
# ==========================================
...
# ==========================================
# 3. Entry point — hybrid dispatch + skip_reduce for large K
# ==========================================
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if M >= 128 and K <= 2048:
        # ---- ASM path: M=256/K=1536 (18.1μs, preshuffle is slower) ----
        A = A.contiguous()
    ...
    if K > 2048:
        # Large K: use skip_reduce to get partials, then sum ourselves
        # This avoids the Triton reduce kernel overhead (~2μs)
        # For M=16,K=7168: partials shape is (NUM_KSPLIT, 16, 2112) in float32
        y_pp = gemm_a16wfp4_preshuffle(A, w, w_scales, dtype=torch.bfloat16, skip_reduce=True)
        return y_pp.sum(dim=0).to(torch.bfloat16)
    else:
        # Small K: NUM_KSPLIT=1, no reduce needed
