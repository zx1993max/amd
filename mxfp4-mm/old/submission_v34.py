...
# ==========================================
# 4. Entry point — direct kernel calls, bypass wrapper overhead
# ==========================================
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if M >= 128 and K <= 2048:
        # ===== M=256, K=1536: ASM path (18.1μs best) =====
    ...
    # ===== Preshuffle path: direct kernel call =====
    # Import kernel directly (bypass wrapper)
    from aiter.ops.triton._triton_kernels.gemm.basic.gemm_a16wfp4 import (
        _gemm_a16wfp4_preshuffle_kernel, _get_config, get_splitk
    )
    ...
    # Preshuffle wrapper logic: N, K transform
    K_half = w.shape[1] // 16  # K_packed / 16
    N_real = (w.shape[0]) * 16

    # Get config
    config, _ = _get_config(M, N_real, K_half, shuffle=True)
    ...
    if NUM_KSPLIT > 1:
        SPLITK_BLOCK_SIZE, BLOCK_SIZE_K, NUM_KSPLIT = get_splitk(K_half, BLOCK_SIZE_K, NUM_KSPLIT)
        config["SPLITK_BLOCK_SIZE"] = SPLITK_BLOCK_SIZE
        config["BLOCK_SIZE_K"] = BLOCK_SIZE_K
        config["NUM_KSPLIT"] = NUM_KSPLIT

    if config["BLOCK_SIZE_K"] >= 2 * K_half:
        config["BLOCK_SIZE_K"] = triton.next_power_of_2(2 * K_half)
        config["SPLITK_BLOCK_SIZE"] = 2 * K_half
        config["NUM_KSPLIT"] = 1
        NUM_KSPLIT = 1
    config["BLOCK_SIZE_N"] = max(config["BLOCK_SIZE_N"], 32)

    if NUM_KSPLIT > 1:
        config["SPLITK_BLOCK_SIZE"] = config.get("SPLITK_BLOCK_SIZE", 2 * K_half)
    else:
        config["SPLITK_BLOCK_SIZE"] = 2 * K_half

    # Allocate output
    if NUM_KSPLIT > 1:
        y_pp = torch.empty((NUM_KSPLIT, M, N_real), dtype=torch.float32, device=A.device)
        c_ptr = y_pp
        stride_ck = y_pp.stride(0)
        stride_cm = y_pp.stride(1)
        stride_cn = y_pp.stride(2)
    else:
        y = torch.empty((M, N_real), dtype=torch.bfloat16, device=A.device)
        c_ptr = y
        stride_ck = 0
    ...
    grid = (
        ...
        * triton.cdiv(N_real, config["BLOCK_SIZE_N"]),
    )

    # Direct kernel launch — no wrapper overhead!
    ...
    if NUM_KSPLIT > 1:
        # torch.sum reduce (faster than Triton reduce kernel by ~0.6μs)
